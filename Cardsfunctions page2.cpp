...
// called in cardsMovement : determine if there are more cards on top of the card that the user is dragging and back up information for both while changing their state
...
int manageBackupcard(Decks c[], int &largestLayer, int &movesCounter, int &score)
{
    ...
    if (c[0].backupDeck.totalLayer > 27) {
        ...
        createCards(c,cardBitmap,largestLayer);
        movesCounter++;
